...
CMD:helpers(playerid, params[])
{
	...
	foreach(new i: Player)
	{
		if(PlayerInfo[i][pHelperLevel] > 0)
		{
			if(HelperDuty[i] == 1)
			{
				format(string, sizeof(string), "Helper Level %d %s (%d) - ON DUTY", PlayerInfo[i][pHelperLevel], PlayerInfo[i][pName], i);
				totalDuty++;
			}
			else
				format(string, sizeof(string), "Helper Level %d %s (%d)", PlayerInfo[i][pHelperLevel], PlayerInfo[i][pName], i);
